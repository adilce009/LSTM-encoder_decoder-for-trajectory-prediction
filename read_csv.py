...
def read_track_csv(arguments):
    ...
    #df2 = pandas.read_csv(arguments["input_path_02_tracks"])

    # Use groupby to aggregate track info. Less error prone than iterating over the data.
    grouped = df.groupby([TRACK_ID], sort=False)
    #grouped1 = df1.groupby([TRACK_ID], sort=False)
    #grouped2 = df2.groupby([TRACK_ID], sort=False)
    #grouped = pandas.concat([grouped1, grouped2], axis=1)
    #grouped = pandas.concat([grouped1, grouped2], axis=1).sum(axis=1)
    ...
    for group_id, rows in grouped:
        bounding_boxes = np.transpose(np.array([rows[X].values,
                                                ...
                                                rows[WIDTH].values,
                                                rows[HEIGHT].values]))
        #center_point() # find the center poing of the vehicle
        center_x = rows[X].values + rows[WIDTH].values/2
        center_y = rows[Y].values + rows[HEIGHT].values/2
